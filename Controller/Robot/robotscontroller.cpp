...
void RobotsController::shortcutAddRobot(){
    ...
    double posX = ((robots.size() + 1) * 200) % 1600;
    double posY = ((robots.size() + 1) * 200) % 1600;

    robotIsAliveSlot("Robot " + ip, ip, "Wifi " + ip, 0, 50);
    emit setPos(QVariant::fromValue(ip), QVariant::fromValue(posX), QVariant::fromValue(posY), QVariant::fromValue(0));
    ...
    if((robots.size() - 1)%3 == 0){
        ...
        emit addPathPoint(QVariant::fromValue(ip), QVariant::fromValue("pathPoint " + ip), QVariant::fromValue(posX + 50 * robots.size()), QVariant::fromValue(posY + 50 * robots.size()), QVariant::fromValue((robots.size() - 1)%3));
    }
    emit displayRobots();
}
