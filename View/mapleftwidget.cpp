...
MapLeftWidget::MapLeftWidget(QMainWindow* parent){
    layout = new QVBoxLayout();

    /// Back button
    QPushButton* backBtn = new QPushButton(QIcon(":/icons/arrowLeft.png"),"Maps");
    backBtn->setStyleSheet ("text-align: left");
    backBtn->setIconSize(parent->size()/10);
    layout->addWidget(backBtn);

    /// Save & load buttons
    QPushButton* saveBtn = new QPushButton(QIcon(":/icons/upload.png"),"Save this map");
    QPushButton* loadBtn = new QPushButton(QIcon(":/icons/download.png"),"Load a map");
