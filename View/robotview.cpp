...
RobotView::RobotView (QSharedPointer<Robot> const &_robot, QGraphicsItem* parent):
    QGraphicsPolygonItem(parent), robot(_robot), selected(false), state(GraphicItemState::NO_STATE), shown(true), lastStage(0)
{
...
RobotView::RobotView (QGraphicsItem* parent): QGraphicsPolygonItem(parent), selected(false), state(GraphicItemState::NO_STATE), shown(true), lastStage(0)
{
}
